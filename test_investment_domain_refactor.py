...
class InvestmentDomainRefactorTests(unittest.TestCase):
    ...
    def test_strategy_context_uses_open_position_cycle_baseline_instead_of_research_level_swing_type(self):
        db_path = self.base / "investment.db"
        asset_repo = AssetRepository(str(db_path))
        analysis_repo = AnalysisRepository(str(db_path), legacy_analysis_db_path="")
        asset_service = AssetService(
            asset_store=asset_repo,
            analysis_store=analysis_repo,
            monitoring_store=MonitoringRepository(str(db_path)),
        )
        ...
        empty_context = analysis_repo.get_latest_strategy_context(
            asset_id=asset_id,
            symbol="600519",
            account_name="默认账户",
        )
        self.assertEqual(empty_context["swing_type"], "")
        self.assertEqual(empty_context["holding_period"], "")

        pending_action_id = asset_repo.create_pending_action(
            asset_id=asset_id,
            action_type="buy",
        ...
        buy_success, _, _ = asset_service.record_manual_trade(
            asset_id=asset_id,
            trade_type="buy",
            quantity=100,
            price=1500.0,
        ...
        self.assertTrue(buy_success)

        open_cycle = asset_repo.get_open_position_cycle(asset_id)
        ...
        holding_context = analysis_repo.get_latest_strategy_context(
            asset_id=asset_id,
            symbol="600519",
            account_name="默认账户",
        )
        ...
        clear_success, _, _ = asset_service.record_manual_trade(
            asset_id=asset_id,
            trade_type="clear",
            quantity=0,
            price=1512.0,
        ...
        self.assertIsNone(asset_repo.get_open_position_cycle(asset_id))

        cleared_context = analysis_repo.get_latest_strategy_context(
            asset_id=asset_id,
            symbol="600519",
            account_name="默认账户",
        )
