    ...
    with right_col:
        ...
        if alerts:
            for alert in alerts[:5]:
                ...
                st.markdown(f"[{level_name}] **{alert.get('title', '')[:20]}...**")
...
def display_analysis_results(result: dict):
    ...
    with cols[4]:
        ...
        st.metric("交易信号", signal, delta=f"置信度:{conf}%")
    ...
    if ai_analysis:
                ...
                with st.expander(f"{sector.get('sector_name', '')} - 详细分析"):
                    ...
                    if drivers:
                        ...
                        for driver in drivers[:3]:
                            impact_text = '正面' if driver.get('impact') == '正面' else '负面'
                            st.write(f"- [{impact_text}] [{driver.get('type', '')}] {driver.get('content', '')}")
                    ...
                    # 投资建议
                    st.info(sector.get('investment_advice', ''))
                    st.warning(sector.get('risk_warning', ''))
        ...
        if recommended_stocks:
            ...
            if stock_recommend.get('risk_warning'):
                st.error(stock_recommend.get('risk_warning', ''))
