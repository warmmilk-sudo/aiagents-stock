...
"""
        return self._call_text(
            system_prompt="你是中国宏观经济分析师，擅长从官方数据中提炼当前经济主线。",
            user_prompt=prompt,
            agent_name="宏观总量分析师",
            agent_role="基于宏观与市场数据研判当前中国宏观环境及其A股含义。",
...
"""
        return self._call_text(
            system_prompt="你是中国政策与流动性分析师，擅长把政策信号映射到A股行业风格。",
            user_prompt=prompt,
            agent_name="政策流动性分析师",
            agent_role="评估政策与流动性环境，并推导对A股估值与风格轮动的影响。",
...
"""
        structured = self._call_json(
            system_prompt="你是A股行业配置分析师，只输出合法JSON。",
            user_prompt=prompt,
            fallback=rule_view,
        )
...
候选股票池：
{json.dumps(stock_candidates, ensure_ascii=False, indent=2)}
...
"""
        ...
        structured = self._call_json(
            system_prompt="你是A股选股分析师，只输出合法JSON。",
            user_prompt=prompt,
            fallback=fallback,
        )
        analysis = self.client.call_api(
...
"""
        return self._call_text(
            system_prompt="你是首席策略官，擅长把宏观、行业和选股结论整合成完整投资框架。",
            user_prompt=prompt,
            agent_name="首席策略官",
            agent_role="整合宏观、政策、行业和选股结论，输出完整投资框架。",
