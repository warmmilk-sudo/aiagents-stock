...
## 🤖 新增--AI盯盘 - AI盘中决策与提醒系统（NEW！）

参照AlphaArena项目，基于 DeepSeek AI 的A股盘中分析系统，支持实时监控、智能决策、价格提醒和手工登记（T+1规则适配）。

**核心功能：**
- ✅ **DeepSeek AI决策** - 利用DeepSeek-V3进行深度技术分析
- ✅ **实时监控** - 24/7持续监控目标股票
- ✅ **手工执行闭环** - 生成买卖待办，由用户手工确认和登记
- ✅ **T+1规则** - 完全遵循A股交易规则
- ✅ **持仓管理** - 记录持仓成本，实时显示盈亏，AI决策考虑持仓情况 ⭐ NEW
- ✅ **邮件/Webhook通知** - 及时接收交易信号
- ✅ **稳定数据源** - 优化Tushare接口，专注技术面分析 ⭐ 2025-10-25
- ✅ **完整任务流程** - 创建任务后需手动启动，启动/停止自动同步状态，用户完全控制 ⭐ 2025-10-26
- ✅ **K线图可视化** - 交互式K线图，AI决策标注，实时更新，一目了然 ⭐ 2025-10-26 NEW
- ✅ **统一配置** - 使用主程序的配置管理系统（数据源、邮件、Webhook）
...
#### 🤖 执行模式（当前）
- **仅提醒**：AI盯盘与价格预警都只生成提醒和待办
- **手工登记**：成交后回到系统登记买卖，同步资产和持仓
...
7. **智能盯盘没有生成买卖建议**
   - 确认 DeepSeek API 配置正确
   - 检查监测服务是否运行
   - 查看是否被“仅交易时段执行”限制
   - 查看最近通知和 AI 决策记录

8. **价格预警没有触发**
   - 检查监控项的进场区间、止盈、止损是否已配置
   - 确认实时价格源是否可用
   - 验证监测服务是否运行
   - 查看通知记录中的错误信息
   - 确认交易时间在交易日内
